"use strict";

/*
Дан массив products, необходимо цену каждого продукта уменьшить на 15% используя
метод forEach.
Обратите внимание, что в массиве должны быть изменены цены продуктов после 
выполнения метода forEach. Не нужно создавать новый массив.
Если сложно работать с методами массива, то можно сделать и обычным циклом.
*/
console.log("Task 3");
const products = [
  {
    id: 3,
    price: 200,
  },
  {
    id: 4,
    price: 900,
  },
  {
    id: 1,
    price: 1000,
  },
];

console.log("Исходные значения:" + products[0].price, products[1].price, products[2].price);

products.forEach((product) => (product.price = product.price * 0.85));
console.log("Измененные значения:" + products[0].price, products[1].price, products[2].price);
